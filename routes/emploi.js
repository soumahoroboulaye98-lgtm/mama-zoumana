...
router.post('/ajouter', protegerAdmin, async (req, res) => {
  try {
    ...
    const [classeExiste, matiereExiste, profExiste] = await Promise.all([
      ...
      pool.query('SELECT 1 FROM utilisateurs WHERE id = $1 AND role = $2', [id_prof, 'prof'])
    ]);
...
// ==================================================
// 📋 EMPLOI DU TEMPS COMPLET — Admin seul
// ==================================================
...
// ==================================================
// 🧑‍🏫 EMPLOI DU TEMPS DU PROFESSEUR — Prof connecté
// ==================================================
router.get('/prof', protegerProf, async (req, res) => {
  try {
    const id_prof = req.user.id;
    console.log("📋 Chargement EDT pour prof :", id_prof);
    ...
    const { rows } = await pool.query(`
      ...
      ORDER BY
        ...
        END, e.heure_debut
    `, [id_prof]);
    console.log(`✅ EDT prof chargé — ${rows.length} séance(s)`);
    res.json({ ok: true, seances: rows });
  } catch (e) {
...
// ==================================================
// 📅 EMPLOI DU TEMPS PAR CLASSE — Publique
// ==================================================
  ...
  try {
    ...
    const { rows } = await pool.query(`
      ...
      ORDER BY
        ...
        END, e.heure_debut
    `, [id_classe]);
    console.log(`✅ EDT classe ${id_classe} chargé — ${rows.length} séance(s)`);
    res.json({ ok: true, seances: rows });
  } catch (e) {
...
// ==================================================
// 📊 EMPLOI DU TEMPS GLOBAL — Publique
// ==================================================
    ...
    const { rows } = await pool.query(`
      ...
      ORDER BY
        CASE e.jour
          ...
          WHEN 'Mercredi' THEN 3 WHEN 'Jeudi' THEN 4
          WHEN 'Vendredi' THEN 5 WHEN 'Samedi' THEN 6 ELSE 7
        END, e.heure_debut, c.libelle_classe
    `);
    console.log(`✅ EDT global chargé — ${rows.length} séance(s)`);
    res.json({ ok: true, seances: rows });
  } catch (e) {
    console.error("❌ ERREUR EDT GLOBAL :", e.code, e.message);
    res.json({ ok: false, erreur: e.message });
  }
});
