...
// ==================================================
// 📚 LISTE GLOBALE — Pour la page emploi-temps-globale
// → CORRIGÉ pour correspondre à la TABLE "emploi" et renvoyer /api/emploi ✅
// ==================================================
  ...
  try {
    const { rows } = await pool.query(`
      SELECT e.id_emploi, e.jour, e.heure_debut, e.heure_fin, e.salle,
             ...
             m.libelle_matiere, m.libelle_matiere_ar,
      ...
      ORDER BY
        ...
        END, e.heure_debut
    `);
    console.log(`✅ Emploi du temps global chargé — ${rows.length} séance(s)`);
    res.json({ ok: true, lignes: rows }); // ✅ Ce que le HTML attend
  } catch (e) {
    console.error("❌ ERREUR /api/emploi :", e.code, e.message);
...
router.post('/ajouter', protegerAdmin, async (req, res) => {
  try {
    ...
    const [classeExiste, matiereExiste, profExiste] = await Promise.all([
      ...
      pool.query('SELECT 1 FROM utilisateurs WHERE id_utilisateur = $1 AND role = $2', [id_prof, 'professeur'])
    ]);
    ...
    await pool.query(`
      INSERT INTO emploi(id_classe, id_matiere, id_prof, jour, heure_debut, heure_fin, salle)
...
router.delete('/:id', protegerAdmin, async (req, res) => {
  try {
    ...
    const { rowCount } = await pool.query(
      'DELETE FROM emploi WHERE id_emploi = $1',
      [id_emploi]
    );
