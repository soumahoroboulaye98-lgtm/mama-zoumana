...
router.get('/toutes-classes', protegerAdmin, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`SELECT id_classe, libelle_classe FROM classes ORDER BY libelle_classe ASC`);
...
router.get('/mes-matieres', protegerProf, async (req, res) => {
  try {
    ...
    if (id_classe && !isNaN(id_classe)) { requete += ` AND a.id_classe = $2`; parametres.push(id_classe); }
...
router.get('/eleves-classe/:id_classe', protegerProf, async (req, res) => {
  try {
    ...
    if (isNaN(id_classe)) return res.json({ ok: false, erreur: "⚠️ Identifiant de classe invalide" });
    const { rows } = await pool.query(`
...
router.post('/saisir', protegerProf, async (req, res) => {
  try {
    ...
    const id_prof = req.user.id;
    if (!id_prof) return res.json({ ok: false, erreur: "⚠️ Profil introuvable. Reconnectez-vous." });
    if (!notes || !Array.isArray(notes) || notes.length === 0) return res.json({ ok: false, erreur: "⚠️ Aucune note à enregistrer" });
    const resultat = [];
    for (const n of notes) {
      const { id_eleve, id_classe, id_matiere, trimestre, annee_scolaire, note1, note2, note3, note4, note5, moyenne_matiere, rang, mention, tableau_honneur, appreciation } = n;
      if (!id_eleve || !id_classe || !id_matiere || !trimestre) return res.json({ ok: false, erreur: "⚠️ Données élève incomplètes" });
      ...
      if (valeurs.some(x => x < 0 || x > 20)) return res.json({ ok: false, erreur: "⚠️ Les notes doivent être entre 0 et 20" });
      const moyenneCalculee = valeurs.length ? Number((valeurs.reduce((a, b) => a + b, 0) / valeurs.length).toFixed(2)) : null;
      const { rows: [coef] } = await pool.query('SELECT coefficient FROM matieres WHERE id_matiere = $1', [id_matiere]);
      ...
      `, [id_eleve, id_matiere, id_classe, trimestre, annee, n1, n2, n3, n4, n5, moyenne_matiere || moyenneCalculee, coefficient, rang || null, mention || null, tableau_honneur || false, appreciation || null, id_prof]);
...
router.post('/consulter', protegerProf, async (req, res) => {
  try {
    ...
    const annee = annee_scolaire || '2026-2027';
    if (!id_classe || !id_matiere || !trimestre) return res.json({ ok: false, erreur: "⚠️ Paramètres manquants" });
    const { rows } = await pool.query(`
