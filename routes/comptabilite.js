...
// ==================================================
router.get('/plan', verifcomptableouadmin, async (req, res) => {
  try {
...
// ==================================================
router.get('/journaux', verifcomptableouadmin, async (req, res) => {
  try {
...
// ==================================================
router.get('/ecritures', verifcomptableouadmin, async (req, res) => {
  try {
...
// ==================================================
router.post('/ecritures/enregistrer', verifcomptableouadmin, async (req, res) => {
  try {
    ...
    if (Math.abs(totalDebit - totalCredit) > 0.01) {
      return res.json({ ok: false, erreur: "❌ Écriture non équilibrée : Débit ≠ Crédit" });
    }
...
// ==================================================
router.post('/ecritures/valider/:id', verifadmin, async (req, res) => {
  try {
    ...
    if (!r.rows.length) return res.json({ ok: false, erreur: "Écriture introuvable ou déjà validée" });
...
// ==================================================
router.get('/balance', verifcomptableouadmin, async (req, res) => {
  try {
...
// ==================================================
router.get('/grand-livre', verifcomptableouadmin, async (req, res) => {
  try {
    const { annee, numero_compte } = req.query;
    let conditions = [`e.statut='valide'`], params = [], idx = 1;
    if (annee) { conditions.push(`EXTRACT(YEAR FROM e.date_ecriture) = $${idx++}`); params.push(annee); }
...
// ==================================================
router.get('/bilan', verifcomptableouadmin, async (req, res) => {
  try {
    const { annee } = req.query;
...
// ==================================================
router.get('/resultat', verifcomptableouadmin, async (req, res) => {
  try {
    const { annee } = req.query;
    const filtreAnnee = annee ? `AND EXTRACT(YEAR FROM e.date_ecriture) = $1` : '';
    const params = annee ? [annee] : [];
...
// ==================================================
router.get('/exercices', verifcomptableouadmin, async (req, res) => {
  try {
...
router.post('/exercices/creer', verifadmin, async (req, res) => {
  try {
...
// ==================================================
// 🔒 CLÔTURE D'EXERCICE — ✅ Admin SEUL
// ==================================================
router.post('/exercices/cloturer/:id', verifadmin, async (req, res) => {
  try {
    ...
    if (!ex.rows.length) return res.json({ ok: false, erreur: "Exercice introuvable" });
    if (ex.rows[0].statut === 'cloture') return res.json({ ok: false, erreur: "Exercice déjà clôturé" });
