...
// ==================================================
// 🔐 MIDDLEWARES DE PROTECTION (avec mode secours)
// ==================================================
...
router.get('/', protegerAdmin, async (req, res) => {
  try {
    ...
    return res.status(500).json({ ok: false, erreur: "⚠️ Impossible de charger les affectations" });
  }
});
...
router.get('/:id', protegerAdmin, async (req, res) => {
  try {
    ...
    if (!affectation)
      return res.status(404).json({ ok: false, erreur: "⚠️ Affectation introuvable" });

    return res.json({ ok: true, affectation });
  } catch (e) {
    console.error("❌ ERREUR détail affectation :", e.code, e.message);
    return res.status(500).json({ ok: false, erreur: "⚠️ Erreur serveur" });
  }
});
...
router.put('/:id', protegerAdmin, async (req, res) => {
  try {
    ...
    if (e.code === '23505')
      return res.status(409).json({ ok: false, erreur: "⚠️ Cette combinaison existe déjà pour cette année !" });
    if (e.code === '23503')
      return res.status(400).json({ ok: false, erreur: "⚠️ L'enseignant, la classe ou la matière n'existe pas" });
    return res.status(500).json({ ok: false, erreur: "⚠️ Impossible de modifier l'affectation" });
  }
});
...
router.delete('/:id', protegerAdmin, async (req, res) => {
  try {
    ...
    if (e.code === '23503')
      return res.status(409).json({ ok: false, erreur: "⚠️ Impossible : utilisée dans des notes ou emplois du temps" });
    return res.status(500).json({ ok: false, erreur: "⚠️ Impossible de supprimer l'affectation" });
  }
});
